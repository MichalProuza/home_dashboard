...
│   ├── school_calendar.json        # Generated by fetch_school_calendar.py
...
└── .github/workflows/
    ...
    ├── school_calendar.yml         # Triggers fetch_school_calendar.py (daily 05:00 UTC)
...
| School events (Plán akcí ZŠ) | `fetchZsCalendar()` | `data/school_calendar.json` |
...
(to handle OAuth2 redirects), then calls all fetch functions via
`Promise.allSettled`, and finally `renderTasks()`. It is called on page load,
every 15 minutes via `setInterval`, and when the tab becomes visible again
after >5 minutes (`visibilitychange`). A `_refreshing` flag prevents
overlapping runs.

Shared helpers at the top of the script: `esc()` (HTML-escapes all external
strings before they are inserted via `innerHTML`) and `fetchJson()` (fetch
with `cache: 'no-store'`, a 15 s timeout, and an HTTP status check). Use both
for any new section.
...
const ZS_CALENDAR_JSON_URL  = './data/school_calendar.json';
